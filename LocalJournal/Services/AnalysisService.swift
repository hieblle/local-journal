...
final class AnalysisService {
    ...
    /// Generate (or regenerate) the report for the period of `kind` that contains
    /// `reference`. Deterministic metrics are always stored; the narrative is
    /// best-effort and skipped when Ollama is offline. Upserts by
    /// (kind, periodStart), so a period is never duplicated. Returns `nil` only
    /// when the period has no entries and `requireEntries` is true.
    @discardableResult
    func generateReport(kind: ReportKind,
                        reference: Date,
                        settings: AppSettings,
                        requireEntries: Bool = true) async -> PeriodicReport? {
        let calendar = Calendar.current
        let allEntries = (try? context.fetch(
            FetchDescriptor<JournalEntry>(sortBy: [SortDescriptor(\.date, order: .forward)])
        )) ?? []

        let metrics = ReportBuilder.metrics(kind: kind, entries: allEntries,
                                            reference: reference, calendar: calendar)
        if requireEntries && !metrics.hasEntries { return nil }

        // Upsert the deterministic snapshot first — it must survive an offline model.
        let report = existingReport(kind: kind, periodStart: metrics.periodStart, calendar: calendar)
            ?? {
                let created = PeriodicReport(kind: kind,
                                             periodStart: metrics.periodStart,
                                             periodEnd: metrics.periodEnd)
                context.insert(created)
                return created
            }()
        report.apply(metrics)
        report.generatedAt = .now
        save()

        // Best-effort narrative layer.
        let service = OllamaService(baseURL: settings.ollamaBaseURL, model: settings.modelName)
        guard await service.isReachable() else { return report }
        ...
        if let raw = try? await service.generate(prompt: LLMPromptTemplates.periodReport(metrics: metrics)) {
            ...
            if !narrative.narrative.isEmpty || !narrative.trajectory.isEmpty {
                ...
                report.trajectory = narrative.trajectory
                report.highlights = narrative.highlights
                report.focus = narrative.focus
                report.recommendations = narrative.recommendations
                report.modelName = settings.modelName
                report.generatedAt = .now
                save()
            }
        }
    ...
    /// On launch: ensure the **last completed** week and month each have a report
    /// (with a narrative when Ollama is reachable). Idempotent — empty periods are
    /// skipped and already-narrated reports are left untouched.
    func autoGenerateDueReports(settings: AppSettings) async {
        let calendar = Calendar.current
        for kind in ReportKind.allCases {
            let reference = ReportBuilder.completedReference(kind: kind, calendar: calendar)
            guard let window = ReportBuilder.interval(kind: kind, containing: reference, calendar: calendar) else { continue }
            if let existing = existingReport(kind: kind, periodStart: window.start, calendar: calendar),
               existing.hasNarrative {
                continue
            }
            await generateReport(kind: kind, reference: reference, settings: settings)
        }
    }

    /// Find a stored report of `kind` whose period starts on the same day as `periodStart`.
    private func existingReport(kind: ReportKind,
                                periodStart: Date,
                                calendar: Calendar) -> PeriodicReport? {
        let all = (try? context.fetch(FetchDescriptor<PeriodicReport>())) ?? []
        return all.first { $0.kind == kind && calendar.isDate($0.periodStart, inSameDayAs: periodStart) }
    }
